...
const getOrCreateDefaultCategory = async (userId: string) => {
...
export const deleteCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const foodItemCount = await prisma.foodItem.count({ where: { categoryId: id } });
    ...
    if (foodItemCount > 0) {
      ...
      if (!fallbackCategory) {
        ...
        let existingFallback = await prisma.category.findFirst({
